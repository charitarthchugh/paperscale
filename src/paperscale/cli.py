...
def build_parser() -> argparse.ArgumentParser:
    ...
    ev.add_argument(
        ...
        type=int,
        default=None,
        help="approx token cap per pplx request (default 32000); smaller prompts let vLLM batch far more of them, at the cost of cross-page conditioning at each chunk boundary",
    )
    ev.add_argument(
        "--no-resume",
        action="store_true",
    ...
    try:
        with rep:
            ...
            with ProcessPoolExecutor(max_workers=n_workers, initializer=_pool_init, initargs=(extra_words,)) as pool:
                ...
                db.mark_docs_done((label, doc, "peer", sha) for label, sums in checksums.items() for doc, sha in sums.items())
                ph.advance()
                ph.done()
            ...
            textlayer_rows, skip = compute_textlayer_agreement(
                tl_pages,
                tl_metas,
                progress=lambda note: ph.advance(),
                jobs=args.jobs,
            ...
            if args.pplx:
                ...
                for label, pages in pplx_todo.items():
                    scored_docs = score_run_pplx(
                        ...
                        # `scored` does what main's inline lambda did -- advance and log --
                        # and additionally refreshes the vllm panel, which is why the
