...
from paperscale.candidates import read_candidates
...
class RunnerConfig:
    ...
    worker_id: str = "local"
    # Document-claim lease/heartbeat (distinct from the page-attempt lease_seconds).
    # Defaults equal the module constants so eager behavior is unchanged; injectable
    # so the lazy candidate handler can prove heartbeat renewal under a tiny lease.
    claim_lease_seconds: float = _CLAIM_LEASE_SECONDS
    claim_heartbeat_seconds: float = _CLAIM_HEARTBEAT_SECONDS
...
class DocumentOcrRunner:
    ...
    def _create_job(self, *, input_path: Path, output_path: Path, job_id: str | None) -> tuple[JobManifest, Json, Any]:
        ...
        # The compact indexes are derived/rebuildable, so they are written without
        # fsync here: at enqueue time the manifest (fsync'd above) is the durable
        # truth and the indexes are trivially regenerated by work/resume. Skipping
        # ~3 fsync barriers per job keeps batch enqueue from being an I/O-bound
        # multi-minute stall before any OCR starts.
        self._write_indexes(manifest, pages, partial=False, fsync=False)
    ...
    def _build_manifest(
        ...
        *,
        ...
        """Assemble a :class:`JobManifest` from explicit fields.

        Shared by ``_create_job`` (eager) and ``materialize_job`` (lazy) so both
        produce byte-identical manifests for the same inputs.
        """
        return JobManifest(
            job_id=job_id,
    ...
    def materialize_job(self, record: "CandidateRecord", *, store: Any) -> JobManifest | None:
        """Lazily create the on-disk job for a planned candidate.

        Tolerates the claim-created job dir (unlike ``_create_job``). If a manifest
        already exists, it is compared field-by-field against the *complete* record
        intent: a full match is a no-op (returns the existing manifest); any mismatch
        is a cross-workload job-id collision (logs an error, returns ``None``).
        Otherwise opens the renderer (runs ``pdfinfo``), builds a deterministic
        manifest stamped with ``record.created_at``, and writes manifest (fsync) then
        indexes (fsync=False). A corrupt/unreadable PDF is marked durably failed and
        returns ``None``. The caller builds its own renderer for processing.
        """
        logger = get_logger()
        profile = get_builtin_profile(record.profile)
        model = record.model or profile.default_model
        manifest_path = self.state.root / self._manifest_rel(record.job_id)
        if manifest_path.exists():
            existing = self._read_manifest(record.job_id)
            if self._manifest_matches_record(existing, record, model):
                return existing
            logger.error(
                "candidate job_id %s collides with divergent manifest; skipping",
                record.job_id,
            )
            return None
        try:
            renderer = self.renderer_factory(Path(record.input_path), dict(profile.render_options))
            page_count = int(getattr(renderer, "page_count"))
        except Exception as exc:  # noqa: BLE001 - corrupt/unreadable PDF is terminal
            store.mark_failed(record.job_id, f"materialize failed: {exc}")
            logger.error("candidate %s: materialize failed: %s", record.job_id, exc)
            return None
        manifest = self._build_manifest(
        ...
        # Manifest is the fsync'd completion barrier; the derived indexes follow
        # without fsync (rebuildable by _pages_from_manifest on a crash).
        self._write_manifest(manifest)
        self._write_indexes(manifest, pages, partial=False, fsync=False)
        return manifest

    @staticmethod
    def _manifest_matches_record(manifest: JobManifest, record: "CandidateRecord", model: str) -> bool:
        return (
            manifest.input_path == str(Path(record.input_path))
            and manifest.output_path == str(Path(record.output_path))
            and manifest.profile == record.profile
            and manifest.model == model
            and manifest.base_url == record.base_url
            and manifest.capacity == record.capacity
        )

    def _pages_from_manifest(self, job_id: str, manifest: JobManifest) -> Json:
        """Rebuild the page map purely from the manifest + on-disk artifacts.

        Used when the status index is missing/corrupt (crash after manifest fsync,
        before index write). Mirrors ``repair_index``'s per-page loop seeded with an
        empty ``old_pages``: an existing page artifact is adopted as succeeded; every
        other page is pending.
        """
        pages: Json = {}
        for page_number in range(1, manifest.page_count + 1):
            ...
            if artifact_path.exists():
                ...
                pages[page_key] = {"state": "pending", "epoch": 0, "attempt_id": None, "fingerprint": None}
        return pages

    def _start_heartbeat(self, store: Any, holder: dict[str, Any], *, interval: float) -> Callable[[], None]:
        """Run a daemon thread that renews the claim until the returned stop is called.

        ``while not stop_event.wait(interval): holder["claim"] = store.heartbeat(...)``.
        The returned callable enforces the strict shutdown order: signal then join.
        The caller releases the claim only *after* this returns, so no tick can
        recreate ``claim.json`` post-release. The holder is shared with the caller so
        heartbeat and release agree on the latest claim epoch.
        """
        ...
        def _loop() -> None:
            while not stop_event.wait(interval):
                ...
                except Exception:  # noqa: BLE001 - a heartbeat miss must not crash the thread
                    return

        thread = threading.Thread(target=_loop, name="claim-heartbeat", daemon=True)
        thread.start()
    ...
    def _process_candidate(self, record: "CandidateRecord", *, store: Any) -> JobStatus | None:
        """Claim, materialize (lazily), and process one planned candidate.

        Skips done (precedence) and failed candidates. Wins the claim, runs a threaded
        heartbeat that spans the sync ``pdfinfo`` materialize *and* the async page pool
        (so a slow materialize cannot lose the lease), then drives processing. Any
        unexpected error marks the job failed and returns ``None`` so one bad candidate
        never aborts the worker or its peers.
        """
        logger = get_logger()
        outcome = store.terminal_outcome(record.job_id)
        if outcome is not None:
            return None  # done (precedence) or failed -> skip; --retry-failed clears elsewhere
        claim = store.try_claim(record.job_id, skip_if_done=True)
        if claim is None:
            return None  # done or owned by a live peer
        holder = {"claim": claim}
        stop_heartbeat = self._start_heartbeat(store, holder, interval=self.config.claim_heartbeat_seconds)
        try:
            manifest_path = self.state.root / self._manifest_rel(record.job_id)
            if manifest_path.exists():
                manifest = self._read_manifest(record.job_id)
                try:
                    pages = self._read_pages_from_status(record.job_id)
                except CompactIndexError:
                    pages = self._pages_from_manifest(record.job_id, manifest)
            else:
                manifest = self.materialize_job(record, store=store)
                if manifest is None:
                    return None  # skipped (collision) or marked failed (corrupt PDF)
                pages = self._pages_from_manifest(record.job_id, manifest)
            ...
            # The threaded heartbeat above already covers processing; pass heartbeat=None
            # into the pool to avoid a redundant second heartbeat task.
            status = self._process_pages(
                manifest, pages, renderer=renderer,
                allow_partial=False, retry_ambiguous=False, heartbeat=None,
            )
            if status.complete:
                store.mark_done(record.job_id)
            return status
        except Exception as exc:  # noqa: BLE001 - isolate one bad candidate from the worker
            store.mark_failed(record.job_id, f"process failed: {exc}")
            logger.error("candidate %s: processing failed: %s", record.job_id, exc)
            return None
        finally:
            stop_heartbeat()
            store.release(holder["claim"])
    ...
    def enqueue_many(
        ...
        logger = get_logger()
        total = len(inputs)
        # Enqueue produces no OCR output, so without this it is an invisible stall;
        # for thousands of PDFs it is the dominant 'startup' time. Log start, a
        # periodic progress line, and a timed completion summary.
        logger.info("enqueuing %d document(s)...", total)
        ...
        for index, input_path in enumerate(inputs, start=1):
            ...
            if now - last_report >= _PROGRESS_INTERVAL_SECONDS:
                logger.info("enqueuing: %d/%d registered (%d skipped)", index, total, len(skipped))
                last_report = now
        logger.info(
            "enqueued %d job(s)%s in %s",
            len(enqueued),
            f", skipped {len(skipped)}" if skipped else "",
            format_duration(float(self.clock()) - started),
        )
    ...
    def work(
        ...
        *,
        ...
        Single snapshot pass: FIRST drain every ``candidates/*.jsonl`` manifest (the
        lazy ``plan`` workflow), materializing-and-processing each candidate, THEN run
        the legacy ``jobs/`` scan for eager workloads. Both phases share one
        ``ClaimStore`` instance; the legacy scan skips any job already processed in the
        candidate phase (a perf/clarity guard — claim + ``terminal_outcome`` are
        idempotent regardless). With no ``candidates/`` dir the behavior is exactly the
        legacy scan (eager back-compat). ``retry_failed`` clears a candidate's durable
        ``failed.json`` before re-attempting it.

        Multiple ``paperscale work`` processes can run concurrently; the ``O_EXCL``
        ClaimStore keeps each job single-owner and reclaims crashed owners at a higher
        epoch. Horizontal scale = many documents across processes.
        """
        store = self._claim_store()
        results: list[JobStatus] = []
        seen: set[str] = set()
        results.extend(
            self._drain_candidates(store, seen, max_jobs=max_jobs, retry_failed=retry_failed)
        )
        ...
        for job_dir in sorted(p for p in jobs_root.iterdir() if p.is_dir()):
            job_id = job_dir.name
            if job_id in seen:
                continue  # already drained from a candidate manifest this pass
    ...
    def _drain_candidates(
        ...
        *,
        ...
        """Materialize-and-process every planned candidate manifest (lazy ``plan`` path).

        Reads each ``candidates/*.jsonl`` in sorted order and routes each record through
        :meth:`_process_candidate` (claim → materialize-or-resume → process →
        mark_done/mark_failed, with per-candidate failure isolation). Tracks processed
        job ids in ``seen`` so the subsequent legacy scan does not revisit them, and
        honors ``max_jobs`` across the whole pass. With ``retry_failed`` set, a
        candidate whose durable outcome is ``failed`` has its ``failed.json`` cleared
        before the claim so the re-attempt proceeds.
        """
        results: list[JobStatus] = []
        candidates_root = self.state.root / "candidates"
        if not candidates_root.exists():
            return results
        for manifest_path in sorted(candidates_root.glob("*.jsonl")):
            try:
                records = read_candidates(manifest_path)
            except (FileNotFoundError, json.JSONDecodeError, ValueError, KeyError):
                continue  # malformed manifest file -> skip, keep draining peers
            for record in records:
                if record.job_id in seen:
                    continue
                seen.add(record.job_id)
                if retry_failed and store.terminal_outcome(record.job_id) == "failed":
                    store.clear_failed(record.job_id)
                status = self._process_candidate(record, store=store)
    ...
    def _claim_store(self) -> Any:
        ...
        return ClaimStore(
            ...
            heartbeat_seconds=self.config.claim_heartbeat_seconds,
        )
